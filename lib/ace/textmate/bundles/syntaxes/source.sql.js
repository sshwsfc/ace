...
, patterns: 
   ...
   , { captures: 
     ...
     , match: '(?i:^\\s*(create)\\s+(aggregate|conversion|database|domain|function|group|(unique\\s+)?index|language|operatorclass|operator|rule|schema|sequence|table|tablespace|trigger|type|user|view)\\s+)([\'"`]?)(\\w+)\\4'
   ...
   , { captures: 
     ...
     , match: '(?i:^\\s*(drop)\\s+(aggregate|conversion|database|domain|function|group|index|language|operatorclass|operator|rule|schema|sequence|table|tablespace|trigger|type|user|view))'
     , name: 'meta.drop.sql'
     }
   , { captures: 
        { '1': { name: 'keyword.other.create.sql' }
   ...
   , { captures: 
     ...
     , match: '(?i:^\\s*(alter)\\s+(aggregate|conversion|database|domain|function|group|index|language|operatorclass|operator|rule|schema|sequence|table|tablespace|trigger|type|user|view)\\s+)'
     , name: 'meta.alter.sql'
     }
   , { captures: 
     ...
     , match: '(?xi)#normalstuff,capture1\\b(bigint|bigserial|bit|boolean|box|bytea|cidr|circle|date|double\\sprecision|inet|int|integer|line|lseg|macaddr|money|oid|path|point|polygon|real|serial|smallint|sysdate|text)\\b#numericsuffix,capture2+3i|\\b(bit\\svarying|character\\s(?:varying)?|tinyint|var\\schar|float|interval)\\((\\d+)\\)#optionalnumericsuffix,capture4+5i|\\b(char|number|varchar\\d?)\\b(?:\\((\\d+)\\))?#specialcase,capture6+7i+8i|\\b(numeric)\\b(?:\\((\\d+),(\\d+)\\))?#specialcase,captures9,10i,11|\\b(times)(?:\\((\\d+)\\))(\\swithoutstimeszone\\b)?#specialcase,captures12,13,14i,15|\\b(timestamp)(?:(s)\\((\\d+)\\)(\\swithoutstimeszone\\b)?)?'
     }
