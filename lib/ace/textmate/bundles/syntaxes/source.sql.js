...
, patterns: 
   ...
   , { captures: 
     ...
     , match: '(?xi)\\b(bigint|bigserial|bit|boolean|box|bytea|cidr|circle|date|double\\sprecision|inet|int|integer|line|lseg|macaddr|money|oid|path|point|polygon|real|serial|smallint|sysdate|text)\\b|\\b(bit\\svarying|character\\s(?:varying)?|tinyint|var\\schar|float|interval)\\((\\d+)\\)|\\b(char|number|varchar\\d?)\\b(?:\\((\\d+)\\))?|\\b(numeric)\\b(?:\\((\\d+),(\\d+)\\))?|\\b(times)(?:\\((\\d+)\\))(\\swithoutstimeszone\\b)?|\\b(timestamp)(?:(s)\\((\\d+)\\)(\\swithoutstimeszone\\b)?)?'
     }
