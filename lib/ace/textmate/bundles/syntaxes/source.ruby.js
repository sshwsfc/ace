...
, patterns: 
   ...
   , { captures: 
     ...
     , match: '(?x)(?=def\\b)#anoptimizationtohelpOnigurumafailfast(?<=^|\\s)(def)\\b#thedefkeyword(\\s+#anoptionalgroupofwhitespacefollowedby\u2026((?>[a-zA-Z_]\\w*(?>\\.|::))?#amethodnameprefix(?>[a-zA-Z_]\\w*(?>[?!]|=(?!>))?#themethodname|===?|>[>=]?|<=>|<[<=]?|[%&`/\\|]|\\*\\*?|=?~|[-+]@?|\\[\\]=?)))?#\u2026oranoperatormethod'
