   ...
   { inline: 
      { patterns: 
         ...
         , { captures: 
           ...
           , match: '(?x)"#Startname,etc(?:#Attributes#Iswear,thisishowthelanguageisdefined,#couldntmakeitupifItried.(?:\\([^)]+\\))?(?:\\{[^}]+\\})?(?:\\[[^\\]]+\\])?#Class,Style,Lang|(?:\\{[^}]+\\})?(?:\\[[^\\]]+\\])?(?:\\([^)]+\\))?#Style,Lang,Class|(?:\\[[^\\]]+\\])?(?:\\{[^}]+\\})?(?:\\([^)]+\\))?#Lang,Style,Class)?([^"]+?)#Linkname\\s?#Optionalwhitespace(?:\\(([^)]+?)\\))?":#Endname(\\w[-\\w_]*)#Linkref(?=[^\\w\\/;]*?(<|\\s|$))#Catchclosingpunctuation#andendofmeta.link'
         ...
         , { captures: 
           ...
           , match: '(?x)"#Startname,etc(?:#Attributes#Iswear,thisishowthelanguageisdefined,#couldntmakeitupifItried.(?:\\([^)]+\\))?(?:\\{[^}]+\\})?(?:\\[[^\\]]+\\])?#Class,Style,Lang|(?:\\{[^}]+\\})?(?:\\[[^\\]]+\\])?(?:\\([^)]+\\))?#Style,Lang,Class|(?:\\[[^\\]]+\\])?(?:\\{[^}]+\\})?(?:\\([^)]+\\))?#Lang,Style,Class)?([^"]+?)#Linkname\\s?#Optionalwhitespace(?:\\(([^)]+?)\\))?":#EndName(\\S*?(?:\\w|\\/|;))#URL(?=[^\\w\\/;]*?(<|\\s|$))#Catchclosingpunctuation#andendofmeta.link'
         ...
         , { captures: 
           ...
           , match: '(?x)\\!#Openimage(\\<|\\=|\\>)?#Optionalalignment(?:#Attributes#Iswear,thisishowthelanguageisdefined,#couldntmakeitupifItried.(?:\\([^)]+\\))?(?:\\{[^}]+\\})?(?:\\[[^\\]]+\\])?#Class,Style,Lang|(?:\\{[^}]+\\})?(?:\\[[^\\]]+\\])?(?:\\([^)]+\\))?#Style,Lang,Class|(?:\\[[^\\]]+\\])?(?:\\{[^}]+\\})?(?:\\([^)]+\\))?#Lang,Style,Class)?(?:\\.[])?#Optional([^\\s(!]+?)#ImageURL\\s?#Optionalspace(?:\\(((?:[^\\(\\)]|\\([^\\)]+\\))+?)\\))?#Optionaltitle\\!#Closeimage(?::(\\S*?(?:\\w|\\/|;))#URL(?=[^\\w\\/;]*?(<|\\s|$))#Catchclosingpunctuation)?'
