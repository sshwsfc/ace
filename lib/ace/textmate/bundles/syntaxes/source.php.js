...
, patterns: 
   [ { captures: 
     ...
     , match: '(?x)(^\\s*)?#1-Leadingwhitespace(#2-meta.embedded.line.empty.php(#3-OpenTag(?:((?<=\\?>)<)#4-Consecutivetags|<)\\?(?i:php|=)?)(\\s*)#5-Loneliness((\\?)>)#6-CloseTag#7-Scope?asscope.php)(\\1#Matchnothingiftherewasno#leadingwhitespace...|(\\s*$\\n)?#ormatchtrailingwhitespace.)'
     }
   , { begin: '^\\s*(?=<\\?)'
         ...
         , { begin: '(?:^\\s*)((?:(?:final|abstract|public|private|protected|static)\\s+)*)(function)(?:\\s+|(\\s*&\\s*))(?:(__(?:call|(?:con|de)struct|get|(?:is|un)?set|tostring|clone|set_state|sleep|wakeup|autoload))|([a-zA-Z0-9_]+))\\s*(\\()'
           ...
           , patterns: 
              ...
              , { captures: 
                ...
                , match: '(?x)\\s*(array)#Typehint\\s*(&)?#Reference\\s*((\\$+)[a-zA-Z_\\x{7f}-\\x{ff}][a-zA-Z0-9_\\x{7f}-\\x{ff}]*)#Thevariablename(?:\\s*(=)#Adefaultvalue\\s*(?i:(NULL)|(\\S.*?))?)?\\s*(?=,|\\))#Aclosingparentheses(endofargumentlist)oracomma'
                , name: 'meta.function.argument.array.php'
                }
              , { captures: 
                ...
                , match: '(?x)\\s*([A-Za-z_][A-Za-z_0-9]*)#Typehintedclassname\\s*(&)?#Reference\\s*((\\$+)[a-zA-Z_\\x{7f}-\\x{ff}][a-zA-Z0-9_\\x{7f}-\\x{ff}]*)#Thevariablename(?:\\s*(=)#Adefaultvalue\\s*(?i:(NULL)|(\\S.*?))?)?\\s*(?=,|\\))#Aclosingparentheses(endofargumentlist)oracomma'
